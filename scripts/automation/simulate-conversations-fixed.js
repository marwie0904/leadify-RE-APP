...
async function typeMessage(page, message) {
  ...
  const textarea = await page.locator('textarea[placeholder*="Type"], textarea[placeholder*="message"], input[type="text"]').first();
  if (await textarea.isVisible()) {
    await textarea.click();
    await textarea.fill(message);
    await wait(500 + Math.random() * 1000); // Random delay 0.5-1.5 seconds
    
    // Try to press Enter or click send button
...
async function simulateConversationsHeadless() {
  ...
  try {
    ...
    const agents = [
      { id: '59f3e30b-107a-4289-905b-92fa46f56e5f', name: 'ResidentialBot', org: 'Prime Residential Realty' },
      { id: 'e4730144-e803-488d-94f4-b119819383ec', name: 'CommercialAssist', org: 'Commercial Property Experts' },
      { id: '2d9dd1b2-2ddb-4e0d-ad79-b26f7dbfcc3a', name: 'LuxuryAdvisor', org: 'Luxury Estate Partners' },
      { id: '7dc1d9cb-b6e6-43d7-bc51-e623f8ca5e85', name: 'RentalHelper', org: 'Urban Rental Solutions' }
    ];
    ...
    for (const agent of agents) {
      console.log(`\n📱 Simulating conversations for ${agent.name} (${agent.org})`);
      ...
      for (const type of conversationTypes) {
        for (let i = 0; i < conversationsPerType; i++) {
          ...
          try {
            // Navigate to the agent's chat page - trying different URL patterns
            const urls = [
              `${BASE_URL}/chat/${agent.id}`,
              `${BASE_URL}/agents/${agent.id}/chat`,
              `${BASE_URL}/embed/${agent.id}`,
            ...
            for (const url of urls) {
              try {
                ...
                // Check if we can find a chat interface
                const hasChat = await page.locator('textarea, input[type="text"]').first().isVisible();
                if (hasChat) {
                  loaded = true;
                  break;
                }
            ...
            if (!loaded) {
              console.log(`  ⚠️  Could not load chat for ${agent.name}, skipping...`);
              continue;
            }
    ...
    console.log(`\n📊 Simulation Summary:`);
    console.log(`✅ Successfully simulated ${successfulConversations}/${totalConversations} conversations`);
    console.log(`   - ${agents.length} agents`);
    console.log(`   - ${totalConversations / agents.length} conversations per agent attempted`);
...
module.exports = { simulateConversationsHeadless };