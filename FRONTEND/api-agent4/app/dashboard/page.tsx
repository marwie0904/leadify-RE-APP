...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Try the new dashboard summary endpoint first (with v1 migration)
      try {
        const endpoint = migrateEndpoint("/api/dashboard/summary")
        console.log('[Dashboard] Fetching from migrated endpoint:', endpoint)
        
        const summaryData = await apiCall(endpoint, {
          headers: authHeaders,
        })
      ...
      const leadsEndpoint = migrateEndpoint("/api/leads")
      const leadsData = await apiCall(leadsEndpoint, {
        headers: authHeaders,
      })
