...
export function HandoffRequestButton({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Map numeric priority to string values expected by backend
      ...
      console.log('[HandoffRequestButton] Requesting handoff with payload:', payload)
      
      // Use the requestHandoff function from the API
      const data = await requestHandoff(conversationId, payload, authHeaders)
      
      console.log('[HandoffRequestButton] Handoff created:', data)
      
      setRequested(true)
      
      // Check if an agent was assigned
      if (data.handoff?.assignedTo) {
        toast.success(`Assigned to ${data.handoff.assignedTo.name}. They will assist you shortly.`)
      } else {
        toast.success('Human agent requested. You will be connected shortly.')
      }
      ...
      console.error('[HandoffRequestButton] Error requesting handoff:', error)
      setError(error instanceof Error ? error.message : 'Failed to request handoff')
      toast.error('Failed to request human agent. Please try again.')
    } finally {
      setLoading(false)
    }
  }
