...
import { migrationRequestInterceptor, getMigrationConfig } from './api/migration-helper'
...
// Add migration interceptor first (highest priority)
apiClient.addRequestInterceptor(migrationRequestInterceptor)

// Log migration configuration on initialization
if (process.env.NODE_ENV === 'development') {
  const migrationConfig = getMigrationConfig()
  console.log('[API Client] Migration configuration:', migrationConfig)
}
...
// Export migration utilities for other agents and debugging
export { 
  getMigrationConfig,
  migrationRequestInterceptor
} from './api/migration-helper'
