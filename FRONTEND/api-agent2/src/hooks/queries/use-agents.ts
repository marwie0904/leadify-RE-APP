...
export function useAgents() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Migration note: This will automatically migrate to GET /api/v1/agents when feature flag enabled
...
export function useAgent(agentId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const headers = await getAuthHeaders()
      // Migration note: This will automatically migrate to GET /api/v1/agents/{agentId} when feature flag enabled
...
export function useAgentStatus(agentId: string, options?: { refetchInterval?: number }) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const headers = await getAuthHeaders()
      // Migration note: This will automatically migrate to GET /api/v1/agents/{agentId}/status when feature flag enabled
...
export function useAgentPerformance(agentId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const headers = await getAuthHeaders()
      // Migration note: This will automatically migrate to GET /api/v1/agents/{agentId}/performance when feature flag enabled
      return queryFn<{
